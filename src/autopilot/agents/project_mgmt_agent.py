...
class ProjectMgmtAgent(BaseAgent):
    ...
    async def handle(self, task: AgentTask) -> ActionRecord:
        ...
        result = {"status": "stubbed", "note": "would create/update ticket in Jira/Linear"}
        return ActionRecord(agent_type=self.agent_type, platform="project_mgmt", action_type="handle_task", payload=task.payload, result=result, created_at=datetime.now(timezone.utc))
