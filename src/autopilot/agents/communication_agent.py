...
class CommunicationAgent(BaseAgent):
    ...
    async def handle(self, task: AgentTask) -> ActionRecord:
        ...
        logger.info("CommunicationAgent handling task %s (slack=%s, teams=%s)", task.id, bool(slack), bool(teams))
        result = {"status": "stubbed", "note": "would send Slack/Teams messages"}
        return ActionRecord(agent_type=self.agent_type, platform="communication", action_type="handle_task", payload=task.payload, result=result, created_at=datetime.now(timezone.utc))
