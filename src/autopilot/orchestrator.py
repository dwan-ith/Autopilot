...
class OrchestratorAgent:
    def __init__(
        self,
        agent_registry: MAASConnectorRegistry,
        store: Store,
        queue: asyncio.Queue,
    ):
    ...
    def route(self, signal: Signal, mission: Mission) -> list[AgentTask]:
        ...
        if "github" in platform or any(
            k in ttype for k in ("push", "pr", "pull", "issue")
        ):
            tasks.append(
                AgentTask(
        ...
        if (
            any(k in platform for k in ("aws", "gcp", "azure"))
            or "cost" in ttype
            or "anomaly" in ttype
        ):
            tasks.append(
        ...
        logger.debug(
            "Orchestrator.route -> %s tasks for mission %s", len(result), mission.id
        )
        return result
